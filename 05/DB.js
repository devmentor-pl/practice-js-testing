export default class DB {
    ...
    insert(data) {
        return new Promise((resolve, reject) => {
            if(data.id) {
                if(typeof data.id !== 'number') { // jeżeli id elementu nie jest numerem
                    this.async(reject,'ID can be only number!');
                    return null; // stop function
                } else if(this._rows.some(item => item.id === data.id)) { // jeżeli id elementu jest już w bazie
            ...
            this.async(() => {
                if(!data.id) { // jeżeli element nie ma id
