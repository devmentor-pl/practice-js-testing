...
describe('RandomNumber', () => {
    ...
    it('should throw exception when any of args isnt a number', () => {
        const min = '';
        const max = 6;
        expect(() => randomNumber(min, max)).toThrow('Incorrect type of property.');
    });

    it('should throw exception when max range is smaller than min range', () => {
        const min = 10;
        const max = 9;
        expect(() => randomNumber(min, max)).toThrow('Incorrect range of numbers.');
    });
    ...
    it('should throw exception when provided argument is not an integer', () =>{
        expect(() => randomNumber(1.2, 3)).toThrow('Incorrect number provided.');
    });
})